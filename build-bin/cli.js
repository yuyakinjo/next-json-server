...
function generateJsonRoute() {
    ...
    // internal.tsの生成
    const internalTsContent = fs.readFileSync(path.join(__dirname, "../templates/internal.ts"), "utf-8");
    fs.writeFileSync(path.join(targetDir, "internal.ts"), internalTsContent);
    console.log("生成しました: internal.ts");
    // route.tsの生成
    const routeTsContent = fs.readFileSync(path.join(__dirname, "../templates/route.ts"), "utf-8");

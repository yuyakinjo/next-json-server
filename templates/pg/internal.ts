...
export async function getAllRecords(tableName: string) {
  try {
    ...
    console.error(`getAllRecords error for table ${tableName}:`, error);
    return [];
  }
}
...
export async function getRecordById(tableName: string, id: string) {
  try {
    ...
    console.error(
      `getRecordById error for table ${tableName}, id ${id}:`,
      error,
    );
    return null;
  }
  ...
  try {
    ...
    for (const [key, value] of searchParams.entries()) {
      if (key.startsWith("gt_")) {
        ...
        conditions.push(
          sql`${sql.identifier(field)} IN (${sql.join(inValues)})`,
        );
      } else {
    ...
    console.error(
      `filterRecordsBySearchParams error for table ${tableName}:`,
      error,
    );
    return [];
  }
...
export async function createRecord(
  ...
  try {
    ...
    console.error(`createRecord error for table ${tableName}:`, error);
    throw error;
  }
}
...
export async function updateRecord(
  ...
  try {
    ...
    console.error(
      `updateRecord error for table ${tableName}, id ${id}:`,
      error,
    );
    throw error;
  }
...
export async function deleteRecord(tableName: string, id: string) {
  try {
    ...
    console.error(
      `deleteRecord error for table ${tableName}, id ${id}:`,
      error,
    );
    throw error;
  }
